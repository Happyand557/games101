...
Eigen::Matrix4f get_projection_matrix(float eye_fov, float aspect_ratio,
  ...
  float radian_eye_fov = eye_fov / 180 * MY_PI;
  Eigen::Matrix4f projection = Eigen::Matrix4f::Identity();
  projection(0, 0) = -1 / (std::tan(radian_eye_fov / 2) * aspect_ratio);
  projection(1, 1) = -1 / (std::tan(radian_eye_fov / 2));
  projection(2, 2) = -(zNear + zFar) / (zFar - zNear);
  projection(2, 3) = (zNear * zFar * 2) / (zNear - zFar);
  projection(3, 2) = 1;
  projection(3, 3) = 0;
